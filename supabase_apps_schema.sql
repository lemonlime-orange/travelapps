...
-- Before You Land content is stored independently from general app settings.
delete from public.app_settings where key = 'before_land';

create table if not exists public.before_land_steps (
...
-- Convert installations that used UUID step IDs while preserving their rows.
do $$
begin
  if exists (
    select 1
    from information_schema.columns
    where table_schema = 'public'
      and table_name = 'before_land_steps'
      and column_name = 'id'
      and data_type = 'uuid'
  ) then
    alter table public.before_land_steps drop column id cascade;
    alter table public.before_land_steps
      add column id bigint generated by default as identity primary key;
  end if;
end
$$;

-- Older installations numbered positions from zero. Shift them once to one-based numbering.
update public.before_land_steps
set position = position + 1
where exists (
  select 1
  from public.before_land_steps
  where position = 0
);

create index if not exists before_land_steps_position_idx
on public.before_land_steps (position);

drop trigger if exists set_before_land_steps_updated_at on public.before_land_steps;

create trigger set_before_land_steps_updated_at
before update on public.before_land_steps
for each row
execute function public.set_updated_at();

alter table public.before_land_steps enable row level security;

drop policy if exists "Allow public before land step reads" on public.before_land_steps;

create policy "Allow public before land step reads"
on public.before_land_steps
for select
using (true);
