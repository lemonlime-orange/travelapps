...
Essential Apps는 일반 카테고리 문자열에 포함되지 않으며 Supabase의 `essential_apps` 테이블에서 `app_id`와 선정 이유(`why_essential`)를 별도 관리합니다. 선정 이유는 Essential Apps 화면의 앱 설명 위에 강조되어 표시됩니다.
...
| 테이블 | 용도 |
|---|---|
| `essential_apps` | Essential Apps로 선택된 앱 ID와 선정 이유 |
